...
InflatedHugePageQueue *ihpq_create(void)
{
	...
	ihpq = (InflatedHugePageQueue *)kmalloc(sizeof(InflatedHugePageQueue),
						GFP_KERNEL);
...
int ihpq_init(InflatedHugePageQueue *ihpq, uint32_t capacity)
{
	...
	ihpq->inflated_huge_pages =
		(hwaddr *)kmalloc(capacity * sizeof(hwaddr), GFP_KERNEL);
	ihpq->size = 0;
	...
	if (ihpq->inflated_huge_pages == NULL) {
		printk(KERN_WARNING
		       "ihpq_init: could not allocate memory for ihpq array \n");
		return -1;
	}
...
void ihpq_enqueue(InflatedHugePageQueue *ihpq, hwaddr gpa_huge_frame)
{
	if (ihpq->size == ihpq->capacity) {
		printk(KERN_WARNING
		       "llfree_balloon_enqueue: queue is full, this should not be possible \n");
		return;
	}
...
hwaddr ihpq_dequeue(InflatedHugePageQueue *ihpq)
{
	...
	if (ihpq->size == 0) {
		printk(KERN_WARNING
		       "llfree_balloon_dequeue: trying to dequeue from empty queue, this should never happen \n");
		return 0;
	}
...
hwaddr ihpq_read_last(InflatedHugePageQueue *ihpq, uint32_t offset)
{
	...
	if (offset > ihpq->size) {
		printk(KERN_WARNING
		       "virtio-llfree-balloon: trying to read past queue size limit\n");
		return 0;
	}
...
void ihpq_remove_last_enqueue(InflatedHugePageQueue *ihpq)
{
	...
	if (ihpq->size == 0) {
		printk(KERN_WARNING
		       "virtio-llfree-balloon: trying to remove element from empty queue");
	}
